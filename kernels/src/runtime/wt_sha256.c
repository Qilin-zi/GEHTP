/* wt_sha256.c — FIPS 180-4 SHA-256 (自包含标量实现) */
#include <string.h>
...
void wt_sha256_final(wt_sha256_ctx* c, uint8_t out[32]) {
    ...
    uint8_t pad = 0x80;
    wt_sha256_update(c, &pad, 1);
    uint8_t z = 0;
    while (c->fill != 56) wt_sha256_update(c, &z, 1);
    uint8_t lenb[8];
    for (int i = 0; i < 8; i++) lenb[i] = (uint8_t)(bits >> (56 - 8 * i));
    wt_sha256_update(c, lenb, 8); /* fill==56, 恰好凑满一块触发 compress */
    for (int i = 0; i < 8; i++) {
