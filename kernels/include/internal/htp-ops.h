...
enum htp_op_code {
    ...
    HTP_OP_SUM_ROWS,
    HTP_OP_SSM_CONV,
    HTP_OP_SSM_CONV_SILU,   // Stage C.3 fused: SSM_CONV output → SiLU in-place (skip conv_output_raw)
    HTP_OP_REPEAT,
    HTP_OP_CUMSUM,
    HTP_OP_FILL,
    HTP_OP_DIAG,
    ...
    // Cross-CDSP synchronization primitives
    HTP_OP_SYNC_SIGNAL,   // producer: flush region + barrier + flag=++seq
    HTP_OP_SYNC_WAIT,     // consumer: spin(flag<target) + barrier + invalidate region

    // Stage C: cooked ExecPlan dispatch — single-op batch carries packed plan
    HTP_OP_COOKED_EXEC_PLAN = 200,
...
// Cross-op weight prefetch (opbatch pipelining): staging reserved at top of VTCM
#define HTP_PF_STAGING_SIZE (2u * 1024 * 1024)
// htp_opbatch_req.flags bit 0: enable cross-op matmul weight prefetch for this batch
#define HTP_OPBATCH_REQ_FLAG_PREFETCH 0x1u
// htp_opbatch_req.flags bit 1: invalidate all DCACHE before processing (dual-CDSP coherency)
#define HTP_OPBATCH_REQ_FLAG_INVAL_DCACHE 0x2u
...
struct htp_opbatch_req {
    ...
    uint32_t flags;       // HTP_OPBATCH_REQ_FLAG_* (was pad/unused)
