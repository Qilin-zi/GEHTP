...
binary_output = False
global calculate_split
calculate_split = False
global padding_to
padding_to = SWIV_SEG_SIZE_MIN
global colocate
colocate = False
...
# ----------------------------------------------------------------------------
# Perform CRC checksum checksum of other segments except swiv segment calculation
# ----------------------------------------------------------------------------


def crc32_calculate_split(image_fp, phdr):
    crc = 0xffffffff
    
    image_fp.seek(phdr.p_offset)
    read_in = image_fp.read(phdr.p_filesz)

    # Fill zero-initialized area if exists
    zero_size = phdr.p_memsz - phdr.p_filesz
    if zero_size > 0:
        read_in += zero_size * b'\x00'
...
def check_shift(elf_hdr, phdr_table):
    ...
    if phdr_end_off == phdr_table[0].p_filesz and phdr_table[0].p_offset == 0:
        ...
        phdr_in_first_seg = False
        need_shift = False
    return [phdr_in_first_seg, need_shift]

# ----------------------------------------------------------------------------
# Get the number of bytes that need to be shifted for each segment
# ----------------------------------------------------------------------------

def get_delta_seg_offset(phdr_entry_size, last_delta_seg_offset, align):
    # delta_seg_offset must be an integer multiple of the align value of the current segment and greater than phdr_entry_size and last_delta_seg_offset
    if align == 0:    
        delta_seg_offset = max(phdr_entry_size, last_delta_seg_offset)
    else:
        delta_seg_offset = max((phdr_entry_size + align - 1) // align * align, (last_delta_seg_offset + align - 1) // align * align)
    return delta_seg_offset

# ----------------------------------------------------------------------------
# Get the shifted phdr table, we will follow out_phdr_table to do the 
# segment copy
# ----------------------------------------------------------------------------
def get_shifted_phdr_table(phdr_in_first_seg, need_shift, elf_header, out_phdr_table):
    if need_shift is True:
        if phdr_in_first_seg is True:
            out_phdr_table[0].p_filesz += elf_header.e_phentsize
            delta_seg_offset = 0
            for phdr in out_phdr_table[1:]:
                delta_seg_offset = get_delta_seg_offset(elf_header.e_phentsize, delta_seg_offset, phdr.p_align)
                phdr.p_offset += delta_seg_offset
        else:
            delta_seg_offset = 0
            for phdr in out_phdr_table:
                delta_seg_offset = get_delta_seg_offset(elf_header.e_phentsize, delta_seg_offset, phdr.p_align)
                phdr.p_offset += delta_seg_offset
    else:
        if phdr_in_first_seg is True:
...
def swiv_generate(input_image, loadable, vaddr, paddr, align, output_image):
    ...
    last_phdr = phdr_table[num_phdrs_without_swiv - 1]
    ...
    # Add a new program header and its SWIV segment only when:
    # - the SWIV context is loadable
    # or
    # - the input image does not contain a section header table.
    #
    # Otherwise, add a section header and its SWIV section.
    out_phdr_table = deepcopy(phdr_table)
    if loadable is True or has_sections == 0:
        [phdr_in_first_seg, need_shift] = check_shift(elf_header, phdr_table)
        if has_sections != 0 and need_shift is True:
            elf_in_fp.close()
            elf_out_fp.close()
            print("ERROR: segment shift is needed, but have sections. It is not supported yet. Abort")
            sys.exit(2)

        get_shifted_phdr_table(phdr_in_first_seg, need_shift, elf_header, out_phdr_table)
        ...
        if has_sections == 0:
            swiv_phdr.p_offset = out_phdr_table[num_phdrs_without_swiv - 1].p_offset + out_phdr_table[num_phdrs_without_swiv - 1].p_filesz
        else:
        ...
        # Make sure alignment
        # Alignment value 0 is valid according to ELF standard, same as alignment 1
        if align > 1:
        ...
        if colocate:
            # Insert the SWIV section at the offset where the SHT originally began.
            # Instead of assuming the SHT starts immediately after the last section,
            # we should respect the actual recorded start of the SHT (e_shoff).
            # This handles cases where there is a 'gap' or alignment padding
            # between the last section data and the SHT.

            # Sanity Check 1: Ensure Section Header Table is at the end of the image layout
            # If SHT is not at the end, shifting it introduces complexity and might overwrite
            # following data
            elf_in_fp.seek(0, 2)
            file_size = elf_in_fp.tell()
            sht_end_offset = elf_header.e_shoff + (elf_header.e_shnum * elf_header.e_shentsize)
            if sht_end_offset != file_size:
                print("ERROR: Section Header Table is not at the end of the input image. "
                      "The 'colocate' feature requires the SHT to be at the end of the file layout. Abort.")
                elf_in_fp.close()
                elf_out_fp.close()
                sys.exit(2)

            # Locate the physical last section
            max_end_offset = 0

            for s in shdr_table_input:
                curr_end = s.sh_offset + s.sh_size
                if curr_end > max_end_offset:
                    max_end_offset = curr_end

            original_sht_start = elf_header.e_shoff
            # Calculate the gap between SHT and last section for logging/debug
            gap_size = original_sht_start - max_end_offset
            if gap_size > 0:
                print("INFO: Gap detected between last section and SHT: %d bytes. "
                      "This gap will be preserved." % gap_size)

            # Locate the SWIV header at the previous SHT start location
            swiv_shdr.sh_offset = original_sht_start

            # Sanity Check 2: Ensure no Program Segment overlaps with the insertion point.
            # The 'colocate' logic inserts the SWIV section at the offset where the SHT originally began.
            # If a PT_LOAD segment (or any other segment) physically covers the SHT area,
            # inserting the SWIV section here would inject data into the middle of that segment,
            # corrupting the segment's integrity without updating p_filesz/p_memsz.

            # The insertion point is effectively the start of the original SHT (old e_shoff),
            # which we previously calculated as swiv_shdr.sh_offset (end of the last physical section).
            insertion_offset_start = swiv_shdr.sh_offset

            for i, ph in enumerate(phdr_table):
                seg_start = ph.p_offset
                seg_end = ph.p_offset + ph.p_filesz

                # Check if the insertion point lies strictly inside a valid segment.
                # Logic: If the insertion point is greater than the start AND less than the end,
                # it means the SHT is currently embedded *inside* a segment.
                if seg_start <= insertion_offset_start < seg_end:
                    print("ERROR: The insertion point (0x%x) is strictly inside Program Segment Index %d (Start: 0x%x, End: 0x%x). "
                            "Using 'colocate' would inject SWIV data into the middle of this segment, corrupting the image. Abort."
                            % (insertion_offset_start, i, seg_start, seg_end))
                    elf_in_fp.close()
                    elf_out_fp.close()
                    sys.exit(2)

            # Move section header offset to accommodate new SWIV section
            elf_header.e_shoff += swiv_shdr.sh_size
        else:
            # Locate the SWIV header just past the end of the section header table
            # (Account for a new entry for swiv header in the table too!)
            swiv_shdr.sh_offset = elf_header.e_shoff + (elf_header.e_shnum + 1) * elf_header.e_shentsize
    ...
    if elf_header.e_phoff != 0:
        ...
        for i in range(num_phdrs_without_swiv):
            # If phdr in first segment and we need add a new entry to phdr, we won't copy first segment here
            if (loadable is True or has_sections == 0) and (phdr_in_first_seg is True and i == 0):
                continue
            src_phdr = phdr_table[i]
            out_phdr = out_phdr_table[i]
            src_offset = src_phdr.p_offset
            out_offset = out_phdr.p_offset

            # Copy the ELF segment
            file_copy_offset(elf_in_fp, src_offset, elf_out_fp, out_offset, src_phdr.p_filesz)
    ...
    if loadable is True or has_sections == 0:
        # We need copy other data in first segment firstly
        if phdr_in_first_seg is True and has_sections == 0:
            file_copy_offset(elf_in_fp, phdr_start, elf_out_fp, phdr_start + phdr_size, phdr_table[0].p_filesz - phdr_start)
    ...
    if has_sections == 1:
        ...
        for i in range(num_shdrs):
            ...
            if curr_shdr.sh_type == SHT_NOBITS or curr_shdr.sh_size == 0:
                continue
    ...
    if loadable is True or has_sections == 0:
        ...
        swiv_seg = SWIV_Seg()
        swiv_seg.crc = crc32_calculate(elf_out_fp, out_phdr_table, num_phdrs_without_swiv, loadable)
        ...
        swiv_seg = SWIV_Seg()
        swiv_seg.crc = crc32_calculate(elf_out_fp, phdr_table, num_phdrs_without_swiv, loadable)
    ...
    if calculate_split is True:
        offset = 0
        elf_out_bfp = open(output_image + "_split.bin", "wb+")

        # If need to add SWIV segment to elf_out_fp, 
        # use out_phdr_table to get the information of each loadable segment,
        # because segments may be shifted.
        if has_sections == 0:
            phdr_table_ptr = out_phdr_table
        else:
            phdr_table_ptr = phdr_table

        for i in range(num_phdrs_without_swiv):
            if phdr_table_ptr[i].p_type != LOAD_TYPE:
                continue
            # Generate separate SWIV meta for each loadable segment
            swiv_seg.crc = crc32_calculate_split(elf_out_fp, phdr_table_ptr[i])
            elf_out_bfp.seek(offset)
            elf_out_bfp.write(swiv_seg.getPackedData())
            offset += swiv_seg.getDataSize()
...
def print_help():
    print (
        "SWIV build utility\n"
        "Usage:\n"
        "   Loadable SWIV metadata:\n"
        "       python3 swiv_build_utility.py -i <input_image> -o <output_image> -v <vaddr> -p <paddr> -l\n"
        "   Non-loadable SWIV metadata:\n"
        "       python3 swiv_build_utility.py -i <input_image> -o <output_image>\n"
        "Options:\n"
        "-h, --help             Print this help.\n"
        "-i, --input_image      Image to encrypt , This parameter is mandatory.\n"
        "-o, --output_image     Output image , This parameter is mandatory.\n"
        "-l, --loadable         Make the SWIV context loadable. Should be omitted if unloadable context is required.\n"
        "-v, --vaddr            Virtual address of the SWIV segment.\n"
        "-p, --paddr            Physical address of the SWIV segment.\n"
        "-a, --align            For loadable SWIV context only. Customize the alignment value(in HEX format) for loadable swiv seg. default: 0x4\n"
        "-d, --paddingto        For loadable SWIV context only. Customize the final SWIV seg size in HEX format. Add padding bytes(0) to increase the seg size to reach the given value, default and minimum value is 0x10\n"
        "-b, --binary           Output the SWIV meta which contains CRC for all loadable segments as binary\n"
        "-s, --split            Output the group of SWIV meta which contains CRCs for per loadable segment as binary\n"
        "-c, --colocate         Co-locate SWIV section with other sections to prevent from relocated by toolchain, ex. Gradle build system. Default behavior is OFF which results in SWIV context at end of file, typically after SHT.\n"
    )
...
def main(argv):
    ...
    align = 0x4
    try:
        opts, args = getopt.getopt(argv, "hi:o:lv:p:a:d:bsc", ["help", "input_image=", "ouput_image=", "loadable", "vaddr=", "paddr=", "align=", "paddingto=", "binary", "split", "colocate"])
    except getopt.GetoptError:
        print('Invalid option. check swiv_build_utility.py -h\n')
        sys.exit(2)

    # print_help() if there is no input parameter
    if len(sys.argv) == 1:
        print_help()
        sys.exit()
    ...
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            ...
            align = str2hex(arg)
        elif opt in ("-d", "--paddingto"):
            global padding_to
            padding_to = str2hex(arg)
            if padding_to < 0x10:
                print('given padding is out of range, should no less than 16')
                sys.exit(2)
            ...
            global calculate_split
            calculate_split = True
        elif opt in ("-c", "--colocate"):
            global colocate
            colocate = True

    if input_image == "" or output_image == "":
        print('input and output file path are mandatory')
        sys.exit(2)
    if loadable is True:
        if vaddr == 0 or paddr == 0:
            print('VA and PA are mandatory for loadable SWIV segment')
            sys.exit(2)
        if calculate_split is True:
            print("loadable option and split option cannot exist at the same time")
            sys.exit(2)

    return swiv_generate(input_image, loadable, vaddr, paddr, align, output_image)
